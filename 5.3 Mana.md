...
* [Section 6.5 - Distributed Random Number Generator](./6.5dRNG): The dRNG committee members must be composed by high consensus Mana nodes.
...
The consensus Mana of a node is only calculated and stored relatively to the end of each epoch (see [Section 4.2 - Timestamps](./4.2Timestamps.md)). Only the values of the last `MAX_STORED_EPOCHS` epochs are stored. Thus, if $t_E$ is the time of the end of epoch $E$, to update the cMana vector from epoch $E-1$ to $E$, a node must perform the following algorithm:
...
Both calculations defined above (cMana and active cMana on an epoch $E$) can only be carried out when epoch $E$ is finalized—that is, at least `TIMESTAMP_CUTOFF` (see [Section 4.2 - Timestamps](./4.2Timestamps.md)) units of time after $t_E$—to make sure that no more messages belonging to the epoch will appear in the network.
