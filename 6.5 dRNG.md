...
|  `TIMESTAMP_CUTOFF`     | duration |  Message timestamp cutoff (Assuming the node is in sync, the time after which point the node will receive no new messages with a particular timestamp which will be finalized)|  2`DLARGE` +`W`= 90[sec]|
 ...
 For more information on the `TIMESTAMP_CUTOFF`  see section [Section 4.2 -  Timestamps](./4.2 Timestamps.md)
...
DRNG payload layout is discussed in [Section 2.3 - Standard Payloads Layout](./2.3 Standard Payloads Layout#2.3.5.5-drng-application-message).
