...
With high probability, we can be sure that for any time `t`, no node can issue a message with timestamp `t` after `t + W + 2*DLARGE`, because after this time, the message would be considered to be bad with level of knowledge 3.  Thus, assuming the node is in sync, see [Section 6.5 - Node Perception Reorganization](./6.5%20Node%20Perception%20Reorganization.md), after approximately 1.5 minutes, the number of messages of a particular timestamp cannot be altered. 
